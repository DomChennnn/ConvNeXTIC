...
def create_lambda_list(max_val, min_val, num=20) -> list:
    step = (max_val - min_val) / num
    lambda_list = [max_val-step*i for i in range(num+1)]
    return lambda_list
...
def parse_args(argv):
    ...
    parser.add_argument('--name', help='result dir name', default='VCIP_VR_new', type=str)
    parser.add_argument('--resume', help='snapshot path', default="/workspace/dmc/ConvNextIC/results/VCIP_VR_new/7/snapshots/best.pt")
...
def test(logger, test_dataloader, model, criterion, metric, lambda_rd_list):
    ...
    lambda_rd = get_lambda_rd_from_numpy([lambda_rd_list], 1)
    # print(lambda_rd)
...
def train(args, config, base_dir, snapshot_dir, output_dir, log_dir):
    ...
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 120], gamma=0.1)
    # AdamW
    ...
    lambda_rd_list = create_lambda_list(config["lambda_max"],config["lambda_min"],20)
    loss_best, bpp_loss, mse_loss = test(logger, test_dataloader, model, criterion, metric, lambda_rd_list[0])
    print(loss_best)
    # if config['set_lr']:
    ...
    for epoch in range(start_epoch,config['epochs']):
        ...
        if epoch >= 0:
            model.update()
            loss, bpp_loss, mse_loss = test(logger, test_dataloader, model, criterion, metric, lambda_rd_list[0])
            if loss < loss_best:
                logging.info('Best!')
