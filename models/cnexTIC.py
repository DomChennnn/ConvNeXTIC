...
# ####################################################scctx and using External Attention##############################################################
# import math
# import torch
# import torch.nn as nn
# from compressai.ans import BufferedRansEncoder, RansDecoder
# from compressai.entropy_models import EntropyBottleneck, GaussianConditional
# from timm.models.layers import trunc_normal_
# from .layers import ConvNeXtLayer, CheckerboardMaskedConv2d, External_attention
# from .utils import update_registered_buffers, Demultiplexer, Multiplexer, conv, deconv, quantize_ste
#
# # From Balle's tensorflow compression examples
# SCALES_MIN = 0.11
...
#                                   is_first = False,
#                                   encode=True)
#         self.g_a_EA0 = External_attention(embed_dim)
#         self.g_a2 = ConvNeXtLayer(dim_in=embed_dim,
#                                   dim_out=embed_dim,
#                                   depth=depths[2],
#                                   drop = dpr[sum(depths[:2]):sum(depths[:3])],
...
#                                   is_first = False,
#                                   encode=True)
#         self.g_a_EA1 = External_attention(latent_dim)
#         self.h_a0 = ConvNeXtLayer(dim_in=latent_dim,
#                                   dim_out=embed_dim,
#                                   depth=depths[4],
#                                   drop = dpr[sum(depths[:4]):sum(depths[:5])],
...
#                                   encode=True,
#                                   is_hyper=True)
#         self.h_a_EA0 = External_attention(embed_dim)
#
#         depths = depths[::-1]
#         self.h_s_EA0 = External_attention(embed_dim)
#         self.h_s0 = ConvNeXtLayer(dim_in=embed_dim,
#                                   dim_out=embed_dim,
#                                   depth=depths[0],
#                                   drop = dpr[sum(depths[:0]):sum(depths[:1])],
...
#                                   is_hyper=True)
#
#         self.g_s_EA0 = External_attention(latent_dim)
#         self.g_s0 = ConvNeXtLayer(dim_in=latent_dim,
#                                   dim_out=embed_dim,
#                                   depth=depths[2],
#                                   drop = dpr[sum(depths[:2]):sum(depths[:3])],
...
#                                   is_first = False,
#                                   encode=False)
#         self.g_s_EA1 = External_attention(embed_dim)
#         self.g_s2 = ConvNeXtLayer(dim_in=embed_dim,
#                                   dim_out=embed_dim,
#                                   depth=depths[4],
#                                   drop = dpr[sum(depths[:4]):sum(depths[:5])],
...
from compressai.layers import MaskedConv2d
from timm.models.layers import trunc_normal_
from .layers import ConvNeXtLayer, CheckerboardMaskedConv2d, ScalingNet
from .utils import update_registered_buffers, Demultiplexer, Multiplexer, conv, deconv, quantize_ste
...
class cnexTIC(nn.Module):
    def __init__(self, config):
        ...
        self.g_a0 = ConvNeXtLayer(dim_in=in_chans,
                                  ...
                                  is_first = True,
                                  encode=True)
        self.g_a_scaling1 = ScalingNet(embed_dim)
        self.g_a1 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=embed_dim,
                                  depth=depths[1],
                                  drop = dpr[sum(depths[:1]):sum(depths[:2])],
                                  is_first = False,
                                  encode=True)
        self.g_a_scaling2 = ScalingNet(embed_dim)
        self.g_a2 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=embed_dim,
                                  depth=depths[2],
                                  drop = dpr[sum(depths[:2]):sum(depths[:3])],
                                  is_first = False,
                                  encode=True)
        self.g_a_scaling3 = ScalingNet(embed_dim)
        self.g_a3 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=latent_dim,
                                  depth=depths[3],
                                  drop = dpr[sum(depths[:3]):sum(depths[:4])],
                                  is_first = False,
                                  encode=True)
        self.g_a_scaling4 = ScalingNet(latent_dim)
        ...
        self.g_s_scaling1 = ScalingNet(latent_dim)
        self.g_s0 = ConvNeXtLayer(dim_in=latent_dim,
                                  dim_out=embed_dim,
                                  depth=depths[2],
                                  drop = dpr[sum(depths[:2]):sum(depths[:3])],
                                  is_first = False,
                                  encode=False)
        self.g_s_scaling2 = ScalingNet(embed_dim)
        self.g_s1 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=embed_dim,
                                  depth=depths[3],
                                  drop = dpr[sum(depths[:3]):sum(depths[:4])],
                                  is_first = False,
                                  encode=False)
        self.g_s_scaling3 = ScalingNet(embed_dim)
        self.g_s2 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=embed_dim,
                                  depth=depths[4],
                                  drop = dpr[sum(depths[:4]):sum(depths[:5])],
                                  is_first = False,
                                  encode=False)
        self.g_s_scaling4 = ScalingNet(embed_dim)
        self.g_s3 = ConvNeXtLayer(dim_in=embed_dim,
                                  dim_out=in_chans,
                                  depth=depths[5],
                                  drop = dpr[sum(depths[:5]):sum(depths[:6])],
    ...
    def g_a(self, x, lambda_rd):
        x = self.g_a0(x)
        x = self.g_a_scaling1(x, lambda_rd)
        x = self.g_a1(x)
        x = self.g_a_scaling2(x, lambda_rd)
        x = self.g_a2(x)
        x = self.g_a_scaling3(x, lambda_rd)
        x = self.g_a3(x)
        x = self.g_a_scaling4(x, lambda_rd)
        return x

    def g_s(self, x, lambda_rd):
        x = self.g_s_scaling1(x, lambda_rd)
        x = self.g_s0(x)
        x = self.g_s_scaling2(x, lambda_rd)
        x = self.g_s1(x)
        x = self.g_s_scaling3(x, lambda_rd)
        x = self.g_s2(x)
        x = self.g_s_scaling4(x, lambda_rd)
        x = self.g_s3(x)
        return x
